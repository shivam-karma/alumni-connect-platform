...
export default function EventCard({ event }) {
  ...
  // prefer dashboard route (works inside authenticated dashboard)
  const dashboardPath = `/dashboard/events/${id}`;
  // optional public fallback
  const publicPath = `/events/${id}`;
      ...
      <div className="flex gap-4">
        ...
        <div className="w-40 flex flex-col justify-between items-end">
          ...
          <div className="mt-auto">
            ...
            <Link to={dashboardPath} className="btn btn-ghost">View Details</Link>
            {/* if you want public link instead, replace with publicPath */}
          </div>
        </div>
      </div>
    </div>
